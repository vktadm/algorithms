...
paths_list = []  # Список всех найденных путей
...
def find_path(cities, city, path, distance):
    ...
    if not new_cities:
        ...
        # Сохраняем найденный путь в список путей.
        paths_list.append([path, distance])
        return
...
for path in sorted(paths_list, key=lambda p: p[1]):
    print(path[0], path[1])
